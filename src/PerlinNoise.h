...
{
	const int PixNum = 256;
  if (!permutationTexture)
  {
    GLubyte pixels[PixNum * PixNum * 4];
    unsigned int index = 0;

    for (unsigned int y = 0; y < PixNum; y++)
      for (unsigned int x = 0; x < PixNum; x++)
      {
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA8, PixNum, PixNum, 0, GL_BGRA, GL_UNSIGNED_BYTE, pixels);
  }
